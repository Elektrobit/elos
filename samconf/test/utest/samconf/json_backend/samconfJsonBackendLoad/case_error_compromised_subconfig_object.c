...
int samconfTestSamconfJsonBackendLoadErrorCompromisedSubconfigObjectSetup(void **state) {
...
int samconfTestSamconfJsonBackendLoadErrorCompromisedSubconfigObjectTeardown(void **state) {
...
void samconfTestSamconfJsonBackendLoadErrorCompromisedSubconfigObject(void **state) {
    samconfJsonBackendLoadTestData_t *data = (samconfJsonBackendLoadTestData_t *)*state;
    samconfConfigStatusE_t result;
    samconfConfig_t *config;

    TEST("samconfJsonBackendLoad");
    SHOULD("%s", "returns error code given by failing samconfConfigNew during subnode creation, type object.");

    MOCK_FUNC_AFTER_CALL(samconfConfigNew, 1);
