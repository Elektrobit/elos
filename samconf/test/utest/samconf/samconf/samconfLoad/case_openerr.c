...
static samconfConfigStatusE_t samconfOpenDummyFail(UNUSED const char *location,
                                                   UNUSED samconfConfigBackend_t *backend) {
    return SAMCONF_CONFIG_ERROR;
}

static const samconfConfigBackendOps_t samconfDummyOps = {
    .open = samconfOpenDummyFail,
};

void samconfTestSamconfLoadOpenError(UNUSED void **state) {
    ...
    SHOULD("%s", "fail on unsuccesful open");
