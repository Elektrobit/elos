...
int samconfTestSamconfBase64DecodeDecodeErrorSetup(UNUSED void **state) {
    return 0;
}

int samconfTestSamconfBase64DecodeDecodeErrorTeardown(UNUSED void **state) {
    return 0;
}

void samconfTestSamconfBase64DecodeDecodeError(UNUSED void **state) {
    ...
    SHOULD("%s", "issue an error on decoding failure");
    ...
    will_return(__wrap_EVP_DecodeBlock, -1);

    MOCK_FUNC_AFTER_CALL(free, 0);
    expect_value(__wrap_free, ptr, allocMem);
