...
int samconfTestSamconfCryptoUtilsVerifyErrorMismatchSetup(UNUSED void **state) {
    return 0;
}

int samconfTestSamconfCryptoUtilsVerifyErrorMismatchTeardown(UNUSED void **state) {
    return 0;
}

void samconfTestSamconfCryptoUtilsVerifyErrorMismatch(UNUSED void **state) {
    ...
    SHOULD("%s", "return specific error if signature doesnt match calculated hash");
    ...
    will_return(__wrap_EVP_DigestVerifyFinal, 0);
    ...
    assert_int_equal(result, SAMCONF_CONFIG_INVALID_SIGNATURE);
}
