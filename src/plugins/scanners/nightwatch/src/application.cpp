...
Application::Application(elosPlugin_t *plugin, elosPublisher *publisher,
                         std::string name, Thresholds thresholds,
                         unsigned long maxMemUsage, double maxCpuLoad)
    : plugin{plugin}, publisher{publisher}, name{std::move(name)}, pid{},
      memUsageKb{0}, cpuLoadPer{0}, applicationThresholds{thresholds},
      maxMemUsage{maxMemUsage}, maxCpuLoad{maxCpuLoad} {}
...
void Application::checkUsages() {
  std::string pidCmd = "pgrep -n " + name + " | tr -d '\\n'";
  pid = execCmd(pidCmd.c_str());
  if (pid.empty()) {
    safuLogWarnF("No Process ID found for process '%s'", name.c_str());
    return;
  }
  if (assignMemUsage() == SAFU_RESULT_FAILED) {
    safuLogErrF("Could not assign mem usage of process '%s'", name.c_str());
    return;
  }
  if (assignCpuLoad() == SAFU_RESULT_FAILED) {
    safuLogErrF("Could not assign cpu load of process '%s'", name.c_str());
    return;
  }

  // check which threshold was reached
  if ((memUsageKb > maxMemUsage * applicationThresholds.memThreshold3) ||
      (cpuLoadPer > maxCpuLoad * applicationThresholds.cpuThreshold3)) {
    createElosEvent("Threshold 3 was reached!",
        (maxMemUsage * applicationThresholds.memThreshold3),
        (maxCpuLoad * applicationThresholds.cpuThreshold3),
        ELOS_SEVERITY_FATAL);
  } else if ((memUsageKb > maxMemUsage * applicationThresholds.memThreshold2) ||
             (cpuLoadPer > maxCpuLoad * applicationThresholds.cpuThreshold2)) {
    createElosEvent("Threshold 2 was reached!",
        (maxMemUsage * applicationThresholds.memThreshold2),
        (maxCpuLoad * applicationThresholds.cpuThreshold2),
        ELOS_SEVERITY_ERROR);
  } else if ((memUsageKb > maxMemUsage * applicationThresholds.memThreshold1) ||
             (cpuLoadPer > maxCpuLoad * applicationThresholds.cpuThreshold1)) {
    createElosEvent("Threshold 1 was reached!",
        (maxMemUsage * applicationThresholds.memThreshold1),
        (maxCpuLoad * applicationThresholds.cpuThreshold1),
        ELOS_SEVERITY_WARN);
  } else {
    createElosEvent("Process is in Threshold",
        (maxMemUsage * applicationThresholds.memThreshold1),
        (maxCpuLoad * applicationThresholds.cpuThreshold1),
        ELOS_SEVERITY_INFO);
  }
...
void Application::createElosEvent(const std::string &message,
                                  const unsigned long memThreshold,
                                  const double cpuThreshold,
                                  elosSeverityE_t severity) {
  safuResultE_t result = SAFU_RESULT_FAILED;
  std::unique_ptr<elosEvent_t> event = std::make_unique<elosEvent_t>();
  ...
  std::string strPayload = "pid:" + pid + ";name:" + name +
                           ";memUsageKB:" + std::to_string(memUsageKb) +
                           ";cpuUsagePer:" + std::to_string(cpuLoadPer) +
                           ";memThreshold:" + std::to_string(memThreshold) +
                           ";cpuThreshold:" + std::to_string(cpuThreshold) +
                           ";message:" + message;
  ...
  result = elosPluginPublish(plugin, publisher, event.get());
  if (result == SAFU_RESULT_FAILED) {
    safuLogErr("Failed to publish event");
  } else if (result == SAFU_RESULT_OK) {
    safuLogDebugF("Event for process '%s' ('%s') was published",
                  name.c_str(), pid.c_str());
  }

  result = elosEventDeleteMembers(event.get());
  if (result == SAFU_RESULT_FAILED) {
