...
static safuResultE_t _pluginLoad(elosPlugin_t *plugin) {
  ...
  if (plugin == NULL) {
    ...
    if ((plugin->config == NULL) || (plugin->config->key == NULL)) {
      ...
      result = elosPluginCreateSubscriber(plugin, &subscriber);
      if (result == SAFU_RESULT_OK) {
        punisher = new (std::nothrow) Punisher(plugin, subscriber);
      } else {
...
static safuResultE_t _pluginUnload(elosPlugin_t *plugin) {
  ...
  if (plugin == NULL || plugin->config == NULL) {
    ...
    free(plugin->data);
    delete punisher;
    result = elosPluginDeleteSubscriber(plugin, subscriber);
    if (result == SAFU_RESULT_FAILED) {
