...
int elosTestElosMessageLogFindEventInvalidRequestFilterErrorSetup(void **state) {
...
int elosTestElosMessageLogFindEventInvalidRequestFilterErrorTeardown(void **state) {
...
void elosTestElosMessageLogFindEventInvalidRequestFilterError(void **state) {
    safuResultE_t ret;
    elosUtestState_t *data = *state;
    ...
    const char findEventsRequest[] =
        "{\
    \"search\": \".event.noname 'sshd' STRCMP\",\
    \"newest\": [0,0],\
    \"oldest\": [0,0]\
    }";
    ...
    data->expectedJsonResponse = json_tokener_parse("{\"error\": \"Failed to fetch filter rules\"}");
