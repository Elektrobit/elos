...
int elosTestElosMessageLogFindEventUntruncatedSuccessSetup(void **state) {
...
int elosTestElosMessageLogFindEventUntruncatedSuccessTeardown(void **state) {
...
void elosTestElosMessageLogFindEventUntruncatedSuccess(void **state) {
    safuResultE_t ret;
    elosUtestState_t *data = *state;

    TEST("elosMessageLogFindEvent");
    SHOULD("%s", "return SAFU_RESULT_OK on fetching successfully an untruncated event list");
