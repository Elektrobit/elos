...
int elosTestElosClientManagerThreadErrMutexUnlock02Setup(void **state) {
...
int elosTestElosClientManagerThreadErrMutexUnlock02Teardown(UNUSED void **state) {
    return 0;
}

void elosTestElosClientManagerThreadErrMutexUnlock02(void **state) {
    elosClientManagerContext_t *context = *state;
    ...
    SHOULD("%s", "return SAFU_RESULT_FAILED when the second pthread_mutex_unlock failed");
