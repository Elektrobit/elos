...
const char *elosStepTypeToString(elosRpnFilterStepType_t type) {
    const char *typeStrings[] = {
        ...
        _NAME_ENTRY(RPNFILTER_STEP_STRCMP),     _NAME_ENTRY(RPNFILTER_STEP_LDP), _NAME_ENTRY(RPNFILTER_STEP_LDV),
        _NAME_ENTRY(PROCESSFILTER_STEP_LDI_EV),
    };
    const char *result = typeStrings[RPNFILTER_STEP_UNDEFINED];

    if (type > RPNFILTER_STEP_UNDEFINED && type < PROCESSFILTER_STEP_TYPE_COUNT) {
...
const char *elosStackTypeToString(elosRpnFilterValueType_t type) {
    const char *typeStrings[] = {
        ...
        _NAME_ENTRY(RPNFILTER_VALUE_U32),       _NAME_ENTRY(RPNFILTER_VALUE_STRING),
    };
...
elosRpnFilterResultE_t elosVerifySteps(elosRpnFilterBuilder_t **data, elosStepTestEntry_t const *entry,
    ...
    for (i = 0; i < entries; i += 1) {
        ...
        // Setup
        PARAM("entry[%lu]: Input '%s' should result in %d steps, %d stack.", i, entry[i].input, entry[i].stepCount,
              entry[i].stackCount);
        result = elosProcessFilterBuilderNew(&param, data);
        ...
        // Cleanup
        result = elosProcessFilterBuilderDelete(*data);
        assert_true(result == RPNFILTER_RESULT_OK);
        *data = NULL;
    }
