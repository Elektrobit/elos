...
int elosTestElosMessageEventSubscribeStrdupFailedSetup(void **state) {
...
int elosTestElosMessageEventSubscribeStrdupFailedTeardown(void **state) {
...
void elosTestElosMessageEventSubscribeStrdupFailed(void **state) {
    ...
    expect_value(__wrap_strdup, string, "");
    will_return(__wrap_strdup, NULL);

    expect_value(__wrap_json_object_put, obj, data->jobj);
    will_return(__wrap_json_object_put, 0);

    expect_string(elosMessageHandlerResponseCreate, errstr, "failed to allocate memory for filter array string");
