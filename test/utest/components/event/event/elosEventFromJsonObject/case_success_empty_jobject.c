...
int elosTestElosEventFromJsonObjectSuccessEmptyJObjectSetup(UNUSED void **state) {
...
int elosTestElosEventFromJsonObjectSuccessEmptyJObjectTeardown(UNUSED void **state) {
...
void elosTestElosEventFromJsonObjectSuccessEmptyJObject(UNUSED void **state) {
    safuResultE_t retval = SAFU_RESULT_FAILED;
    elosTestState_t *test = *(elosTestState_t **)state;
    elosEvent_t expected = {0};

    TEST("elosEventFromJsonObject");
    SHOULD("%s", "convert json object to event when jobject is empty");

    test->object = json_tokener_parse("{}");
    ...
    assert_int_equal(test->result->source.appName, expected.source.appName);
    assert_int_equal(test->result->source.fileName, expected.source.fileName);
