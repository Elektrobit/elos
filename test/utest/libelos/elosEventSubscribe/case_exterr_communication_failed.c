...
int elosTestElosEventSubscribeExterrCommunicationFailedSetup(void **state) {
...
int elosTestElosEventSubscribeExterrCommunicationFailedTeardown(void **state) {
    struct _TestState *testState = *state;
...
void elosTestElosEventSubscribeExterrCommunicationFailed(void **state) {
    TEST("elosEventSubscribe");
    SHOULD("%s", "Create a subscription message and check if receiving the response fails.");
    ...
    elosMockSendAndReceiveJsonMessage(testState, SAFU_RESULT_FAILED);
