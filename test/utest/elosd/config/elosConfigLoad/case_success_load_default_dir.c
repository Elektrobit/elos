...
int elosTestElosConfigLoadDefaultDirSuccessSetup(UNUSED void **state) {
    return 0;
}

int elosTestElosConfigLoadDefaultDirSuccessTeardown(UNUSED void **state) {
    return 0;
}

int elosTestDummyDirStat(UNUSED const char *pathname, struct stat *statbuf) {
...
void elosTestElosConfigLoadDefaultDirSuccess(UNUSED void **state) {
    ...
    TEST("elosConfigLoad");
    SHOULD("load configs from the given dir");
    ...
    MOCK_FUNC_AFTER_CALL_WITH(stat, 0, elosTestDummyDirStat, NULL);
