...
int elosTestElosConfigLoadDefaultConfigSuccessSetup(UNUSED void **state) {
    return 0;
}

int elosTestElosConfigLoadDefaultConfigSuccessTeardown(UNUSED void **state) {
    return 0;
}

int elosTestDummyConfigStat(UNUSED const char *pathname, struct stat *statbuf) {
...
void elosTestElosConfigLoadDefaultConfigSuccess(UNUSED void **state) {
    ...
    MOCK_FUNC_AFTER_CALL_WITH(stat, 0, elosTestDummyConfigStat, NULL);
    ...
    expect_string(__wrap_dirname, path, "/etc/elos/elosd.json");
