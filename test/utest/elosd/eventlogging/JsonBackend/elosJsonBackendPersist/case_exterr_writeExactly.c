...
int elosTestElosJsonBackendPersistExterrWriteExactlySetup(UNUSED void **state) {
    ...
    char filepattern[] = "/tmp/elosd_2023-04-05.log";
    char filename[] = "/tmp/elosd_2023-04-05.log.0";
...
int elosTestElosJsonBackendPersistExterrWriteExactlyTeardown(UNUSED void **state) {
...
static void _testelosJsonBackendPersistExterrWriteExactlyParam(void **state, int callToFail) {
    elosTestState_t *testState = *state;
    elosJsonBackend_t *jsonBackend = testState->backend->backendData;
    ...
    will_return(__wrap_safuWriteExactly, callToFail == 1 ? -1 : (ssize_t)strlen(testState->expectedString));

    if (callToFail == 2) {
        expect_value(__wrap_safuWriteExactly, fd, WRITE_FD);
        expect_string(__wrap_safuWriteExactly, buf, "\n");
        expect_value(__wrap_safuWriteExactly, len, 1);
        will_return(__wrap_safuWriteExactly, callToFail == 2 ? -1 : 1);
    }
...
void elosTestElosJsonBackendPersistExterrWriteExactly(void **state) {
    _testelosJsonBackendPersistExterrWriteExactlyParam(state, 1);
    _testelosJsonBackendPersistExterrWriteExactlyParam(state, 2);
}
