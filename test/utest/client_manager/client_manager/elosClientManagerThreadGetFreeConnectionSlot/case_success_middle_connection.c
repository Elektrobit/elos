...
int elosTestElosClientManagerThreadGetFreeConnectionSlotSuccessMiddleSetup(void **state) {
    elosClientManagerContext_t *context = *state;
    memset(context, '\0', sizeof(elosClientManagerContext_t));

    for (int i = 0; i < CLIENT_MANAGER_MAX_CONNECTIONS / 2; i += 1) {
...
int elosTestElosClientManagerThreadGetFreeConnectionSlotSuccessMiddleTeardown(UNUSED void **state) {
    return 0;
}

void elosTestElosClientManagerThreadGetFreeConnectionSlotSuccessMiddle(void **state) {
    elosClientManagerContext_t *context = *state;
