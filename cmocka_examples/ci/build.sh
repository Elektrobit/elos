...
if [ $OPTION_PACKAGE -eq 1 ]; then
    CMAKE_PARAM="${CMAKE_PARAM} -DPACKAGING"
    BUILD_TYPE=Release
    OPTION_CLEAN=1
fi
...
#DEPENDENCIES="safu samconf cmocka_mocks cmocka_extensions"
...
ninja -C $CMAKE_BUILD_DIR $NINJA_PARAM all 2>&1 | tee $RESULT_DIR/build_log.txt
