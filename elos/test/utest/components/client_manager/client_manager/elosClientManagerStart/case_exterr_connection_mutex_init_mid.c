...
#define TEST_NUM (CLIENT_MANAGER_MAX_CONNECTIONS / 2)

int elosTestElosClientManagerStartExtErrConnectionMutexInitMidSetup(void **state) {
    elosClientManagerStartUtestInitParameters(state);
    return 0;
}

int elosTestElosClientManagerStartExtErrConnectionMutexInitMidTeardown(void **state) {
...
void elosTestElosClientManagerStartExtErrConnectionMutexInitMid(void **state) {
    elosUteststateT_t *testState = *state;
    int retval = 0;

    TEST("elosClientManagerStart");
    SHOULD("%s", "return -1 when pthread_mutex_init for one of the middle connection locks fails");
