...
int elosTestElosMessageEventQueueReadErrResponseSendSetup(void **state) {
    elosMessageEventQueueReadUtestSetupEventProcessor(state);

    return 0;
}

int elosTestElosMessageEventQueueReadErrResponseSendTeardown(void **state) {
...
void elosTestElosMessageEventQueueReadErrResponseSend(void **state) {
    elosUnitTestState_t *test = *(elosUnitTestState_t **)state;
    ...
    expect_value(elosMessageHandlerSendJson, messageId, ELOS_MESSAGE_RESPONSE_EVENTQUEUE_READ);
