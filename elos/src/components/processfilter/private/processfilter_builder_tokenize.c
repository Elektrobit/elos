...
elosRpnFilterResultE_t elosProcessFilterBuilderTokenizeParse(size_t *startIdx, elosRpnFilterBuilder_t *data) {
    ...
    if ((startIdx == NULL) || (data == NULL) || (data->param.filterString == NULL)) {
        ...
        char const *procLocStart[] = {".process.", ".proc.", ".p."};
        char const *str = data->param.filterString;
        size_t idx = *startIdx;

        result = RPNFILTER_RESULT_NOT_FOUND;

        for (size_t i = 0; i < ARRAY_SIZE(procLocStart); i += 1) {
            size_t len = strlen(procLocStart[i]);
            int retVal;

            retVal = strncmp(procLocStart[i], &str[idx], len);
            if (retVal == 0) {
                idx += len;
                ...
                elosRpnFilterBuilderTokenEntry_t entry = {
                    ...
                    .type = PROCESSFILTER_TOKEN_PROCESS,
                };
...
elosRpnFilterResultE_t elosProcessFilterBuilderTokenize(elosRpnFilterBuilder_t *data) {
    ...
    if (data == NULL) {
        ...
        elosRpnFilterBuilderTokenizeParseFunc_t parseFunc[] = {
            elosProcessFilterBuilderTokenizeParse,
